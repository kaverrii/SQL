...
public class Ring {
 ...
 // sorting the processes based on the id
 for (i = 0; i < num - 1; i++) {
 for (j = 0; j < num - 1; j++) {
 ...
 int ch1;
 int arr[] = new int[10];
 // Mark the last process as inactive
 proc[num - 1].state = "inactive";
 System.out.println("\nProcess " + proc[num - 1].id + " selected as coordinator");
 while (true) {
 System.out.println("\n1. Election 2. Quit");
 ch = in.nextInt();
 // Reset flag for each process
 for (i = 0; i < num; i++) {
 proc[i].f = 0;
 }
 switch (ch) {
...
proc[temp1].id);
 ...
 // Coordinator is found, printing on console
...
class Rr {
 ...
 public int f;
 String state; // indicates whether the process is active or inactive
}
